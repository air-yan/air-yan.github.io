...
具体来说，如果我们有一封已被标记为“运动”分类的邮件，则“篮球”一词的出现与“扣篮”一词的出现相互独立。基于以上假设，我们可以独立地对每个单词进行建模，我们可以将它建模为伯努利分布。我们当然知道这个假设也许是错误的，这就是我们称之为Naive的原因（Naive是朴素贝叶斯中朴素的英文，它也有天真的、无知的、幼稚的意思）。但是，根据我个人的经验，朴素贝叶斯将给你提供相当不错的结果。但是如果你打算删除此假设的话，则需要对依赖性进行大量的额外计算。
