...
![image001.png](/_posts/001.png)

如上所述，μ是期望值。

向量值随机变量Z的协方差为：

![image003.png](/_posts/003.png)


下图显示了均值为零但不同协方差的几个密度函数。

![image005.png](/_posts/005.png)


以下为上图的协方差（从左到右）：

![image007.png](/_posts/007.png)
...
![image011.png](/_posts/011.png)

在上面的等式中，我们插入各个分布而不指明任何类，我们仅将它们抽象为k。所以我们有：

![image013.png](/_posts/013.png)


现在，我们需要对每个参数进行取导，然后将它们设为零找到 argmax（函数值最大时对应的输入值x）。 一些可能对推导有用的公式列举如下：

![image015.png](/_posts/015.png)（如果A是对称的并且与x相互独立）




![image017.png](/_posts/017.png)
-----------------

**证明：**
矩阵A是对称矩阵，所以 *A*= *A*<sup>T</sup>并假设空间维度为n。

![image019.png](/_posts/019.png)

雅可比公式：

![image021.png](/_posts/021.png)

**证明：**

![image023.png](/_posts/023.png)

**证明：**

这个证明有些复杂。你应该事先了解克罗内克函数和Frobenius内部乘积。对于矩阵X，我们可以写成：

![image025.png](/_posts/025.png)

你可以将H视为Frobenius内积的标识元素。在开始证明之前，让我们准备好去找逆矩阵的导数。也就是说，∂X<sup>-1</sup>/∂X。

![image027.png](/_posts/027.png)



所以我们可以这么解：

![image029.png](/_posts/029.png)

接着，让我们回到正题：

![image031.png](/_posts/031.png)

其中F表示Frobenius内积。

接着，带回到原始公式：

![image033.png](/_posts/033.png)
...
![image035.png](/_posts/035.png)


对 μk取导并设为0：

![image037.png](/_posts/037.png)


对 Σ 取导并设为0:

![image039.png](/_posts/039.png)


结果如图所示：

![image041.png](/_posts/041.png)
...
于是，我们有：![image045.png](/_posts/045.png)
...
![image049.png](/_posts/049.png)


为了预测新样本，我们可以使用**贝叶斯法则**来计算P（y = 1 | x）并比较哪个更高。

![image051.png](/_posts/051.png)
